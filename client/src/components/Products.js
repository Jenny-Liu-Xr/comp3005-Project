...
function Products() {
	...
	const deleteProduct = (productId) => {
		if (window.confirm('Do you want to delete the book?')) {
			fetch('http://localhost:3001/books/'+ productId, {
				method: 'DELETE'
			})
				.then(res => {
		...
		<Grid height="100%">
			<Grid display="flex" justifyContent="space-between">
				<Box>
					...
					<Button variant="contained" sx={{marginLeft: 1}} onClick={getProducts}>Search</Button>
				</Box>
				<Box>
					...
					<Table sx={{ minWidth: 650 }} aria-label="simple table">
						...
						<TableBody>
							{filterProducts.slice((pageIndex - 1) * 10, pageIndex * 10).map((product) => (
								...
								>
									<TableCell><img src={BookImage} style={{width: 100, height: 100, objectFit: 'cover'}} /></TableCell>
									...
									<TableCell>
										<Button variant="contained" size="small" onClick={() => toEditProduct(product.ISBN)}>Edit</Button>
										<Button variant="contained" size="small" color="error" sx={{marginLeft: 1}} onClick={() => deleteProduct(product.ISBN)}>
											Delete
										</Button>
									</TableCell>
								</TableRow>
